...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const chars = Array.from(data.subheadline); // Unicode-safe
    let isCancelled = false;

    async function typeText() {
      setTypedSubheadline(""); // Reset
    ...
    return () => {
      isCancelled = true; // Clean up if unmounted
    };
  }, [data?.subheadline]);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 text-center px-6 max-w-2xl">
        ...
        <p className="text-lg md:text-2xl text-white/90 mb-8 leading-relaxed min-h-[3rem]">
          {typedSubheadline || "Ervaar professionele nagelverzorging in een stijlvolle omgeving."}
        </p>
            ...
            {data.socials.map((item, index) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-9 h-9 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition hover:scale-110"
                >
                  {Icon}
                </a>
              );
